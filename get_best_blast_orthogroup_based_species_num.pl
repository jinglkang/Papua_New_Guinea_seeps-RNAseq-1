...
#    my @specs=("Blenny","Blue_eyed","Common","Yaldwyn"); # get the best hit of each protein of these species;
    ...
    foreach my $key (keys %info1_1) {  # select if the protein has a hit to all species
        if (exists $info1_1{$key}->{num_spec} && $info1_1{$key}->{num_spec}==@specs-1 && $info1_1{$key}->{eval} <=1E-30) {
